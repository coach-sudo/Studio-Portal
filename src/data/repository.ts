...
export async function loadStudioSnapshot(role: Role = "coach", studentId?: string): Promise<StudioSnapshot> {
  ...
  const [membership, studio, students, lessons, notes, assignments, materials, links, packages, packageDefinitions, pricingRules, credits, payments, profiles, readers, outbox, recommendations, bookingServices, availabilityRules, availabilityExceptions, serviceOfferings, recurringSeries, bookings, lessonParticipants, integrationImports] = await Promise.all([
    ...
    supabase.from("service_offerings").select("*"), supabase.from("recurring_series").select("*"), supabase.from("bookings").select("*"), supabase.from("lesson_participants").select("*"), supabase.from("integration_imports").select("*").order("created_at",{ascending:false}),
  ]);
  const failed = [membership,studio,students,lessons,notes,assignments,materials,links,packages,packageDefinitions,pricingRules,credits,payments,profiles,readers,outbox,recommendations,bookingServices,availabilityRules,availabilityExceptions,serviceOfferings,recurringSeries,bookings,lessonParticipants,integrationImports].find((result) => result.error);
  ...
  return {
    ...
    materials: (materials.data ?? []).map((r:any)=>{const link=materialLinks.find((l:any)=>l.material_id===r.id);return{id:r.id,studentId:r.owner_student_id,lessonId:link?.lesson_id,title:r.title,category:r.category,role:link?.role??"library",status:r.status,approvalStatus:r.approval_status,storagePath:r.storage_path,externalUrl:r.external_url,caption:r.caption,mimeType:r.mime_type,fileSizeBytes:r.file_size_bytes==null?undefined:Number(r.file_size_bytes),mediaKind:r.media_kind,publicEmbed:r.public_embed,sortOrder:r.sort_order,version:r.version,updatedAt:r.updated_at}}),
    ...
    serviceOfferings:(serviceOfferings.data??[]).map((r:any)=>({id:r.id,version:r.version,updatedAt:r.updated_at,studioId:r.studio_id,serviceId:r.service_id,title:r.title,startsAt:r.starts_at,endsAt:r.ends_at,enrollmentClosesAt:r.enrollment_closes_at,capacity:r.capacity,enrolled:r.enrolled,lessonIds:r.lesson_ids,published:r.published})),
    ...
    bookings:(bookings.data??[]).map((r:any)=>({id:r.id,version:r.version,updatedAt:r.updated_at,studioId:r.studio_id,reference:r.reference,serviceId:r.service_id,offeringId:r.offering_id,seriesId:r.series_id,studentId:r.student_id,guestName:r.guest_name,guestEmail:r.guest_email,guardianName:r.guardian_name,guardianEmail:r.guardian_email,forMinor:r.for_minor,startsAt:r.starts_at,endsAt:r.ends_at,timezone:r.timezone,location:r.location,inPersonLocation:r.in_person_location,locationConfirmedAt:r.location_confirmed_at,status:r.status,paymentPolicy:r.payment_policy,paymentStatus:r.payment_status,totalMinor:Number(r.total_minor),paidMinor:Number(r.paid_minor),currency:r.currency,policySnapshot:r.policy_snapshot,pricingSnapshot:r.pricing_snapshot,balanceDueAt:r.balance_due_at,autoChargeBalance:r.auto_charge_balance,adminOverride:r.admin_override,rescheduleCount:r.reschedule_count})),
