...
export function CoachHome() {
  ...
  const reviewCount = data.integrationImports.filter((item)=>item.status === "needs_review").length + data.materials.filter((item)=>item.approvalStatus === "pending_review").length + data.actorProfiles.filter((item)=>item.status === "review_requested").length + data.assignments.filter((item)=>item.helpRequested).length + data.bookings.filter((item)=>item.status === "needs_attention").length;
    ...
    <div className="home-dashboard-grid">
      ...
      <Section title="Today is your action queue">
        <div className="attention-card">
          <Waypoints />
          <div>
            <strong>{reviewCount + data.recommendations.length} items need attention</strong>
