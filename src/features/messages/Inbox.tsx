...
function threadCandidates(data: StudioSnapshot, role: Role): Thread[] {
  ...
  const portalCoachTitle = data.settings.coachName || "Your coach";
  const portalCoachSubtitle =
    data.settings.coachTitle || "Private studio conversation";
  for (const conversation of data.conversations) {
    const student = data.students.find(
      (item) => item.id === conversation.studentId,
    );
    const state = data.conversationStates.find(
      (item) => item.conversationId === conversation.id,
    );
    const latest = [...data.conversationMessages]
      .reverse()
      .find((item) => item.conversationId === conversation.id);
    known.set(
      conversation.kind === "direct"
        ? `student:${conversation.studentId}`
        : `offering:${conversation.offeringId}`,
      {
        ...conversation,
        title:
          role !== "coach" && conversation.kind === "direct"
            ? portalCoachTitle
            : conversation.title,
        subtitle:
          conversation.kind === "class"
            ...
            : role !== "coach"
              ? portalCoachSubtitle
              : student?.email || "Private conversation",
        unread: Boolean(
          latest &&
          latest.authorRole !== role &&
          (!state?.lastReadAt || latest.createdAt > state.lastReadAt),
        ),
  ...
  const students =
    role === "coach"
      ? data.students.filter((item) => !item.deletedAt)
      : data.students;
  ...
  for (const offering of data.serviceOfferings.filter(
    (item) => item.published,
  )) {
...
const emailTemplates = {
  ...
  reminder: {
    ...
    subject: "Your upcoming Coach'D lesson",
    body: "A quick reminder about your upcoming lesson. Open your portal for the current time, joining link, and lesson work.",
  },
  schedule: {
    label: "Schedule update",
    subject: "Your Coach'D schedule was updated",
    body: "Your lesson schedule has changed. Open your portal to review the current date, time, and lesson details.",
  },
  work: {
    label: "Notes or materials",
    subject: "New lesson work in Coach'D",
    body: "New notes or materials are ready in your portal.",
  },
  package: {
    label: "Package reminder",
    subject: "Your Coach'D lesson package",
    body: "Your lesson package needs attention. Open Payments in your portal to review the balance and renewal options.",
  },
  access: {
    label: "Account access",
    subject: "Your Coach'D portal access",
    body: "Your Coach'D portal account is ready. Use the most recent secure login instructions sent to this address.",
  },
} as const;
...
function Inbox({
  ...
  const initial =
    threads.find((item) => item.id === requested) ||
  ...
  const selected = selectedKey
    ? threads.find((item) => item.id === selectedKey) || initial
    : undefined;
  ...
  const [emailTemplate, setEmailTemplate] =
    useState<keyof typeof emailTemplates>("custom");
  const selectedStudent = data.students.find(
    (item) => item.id === selected?.studentId,
  );
  const [email, setEmail] = useState({
    recipient: params.get("recipient") || selectedStudent?.email || "",
    subject: "",
    body: "",
  });
    ...
    return threads
      .filter((thread) =>
        threadFilter === "unread"
          ? thread.unread
          : threadFilter === "drafts"
            ? Boolean(thread.draftBody)
            : thread.unread ||
              Boolean(thread.draftBody) ||
              (Boolean(thread.lastMessageAt) &&
                new Date(thread.lastMessageAt).getTime() >= recentCutoff) ||
              (role !== "coach" && !thread.lastMessageAt),
      )
      .slice(0, 30);
  ...
  const recipients = useMemo(() => {
    ...
    const students = data.students
      ...
      .map((student) => {
        const household = data.linkedContacts.filter(
          (contact) =>
            contact.studentId === student.id && contact.portalEnabled,
        );
        const searchText = [
          student.fullName,
          student.preferredName,
          student.email,
          ...household.flatMap((contact) => [
            contact.fullName,
            contact.email,
        ...
        return {
          ...
          subtitle: [
            student.email,
            household.length
              ? `${household.length} household contact${household.length === 1 ? "" : "s"}`
              : "",
          ]
            .filter(Boolean)
            .join(" · "),
    ...
    const classes = data.serviceOfferings
      .filter((item) => item.published)
      .map((offering) => ({
        key: `offering:${offering.id}`,
        title: offering.title,
        subtitle: "Group class",
        searchText: `${offering.title} group class`.toLowerCase(),
        thread: threads.find((item) => item.offeringId === offering.id) || {
  ...
  const messages =
    selected &&
    !selected.id.startsWith("student:") &&
    !selected.id.startsWith("offering:")
  ...
  useEffect(() => {
    ...
    if (isDemo) {
      store.transact((draft) => {
        const state = draft.conversationStates.find(
          (item) => item.conversationId === selected.id,
        );
        if (state) {
      ...
      void studioCommand("messages", {
        command: "mark_read",
        entityId: selected.id,
        expectedVersion: 0,
        payload: {},
        reason: "Opened inbox conversation",
      })
        .then(() => invalidateStudioDomains(queryClient, ["messaging"]))
        .catch(() => undefined);
  }, [selected?.id]);
  ...
  const sendMessage = async (event: FormEvent) => {
    ...
    try {
      if (isDemo) {
        ...
        store.transact((draft) => {
          if (
            conversationId.startsWith("student:") ||
            conversationId.startsWith("offering:")
          ) {
  ...
  const undoLastMessage = async () => {
    ...
    try {
      if (isDemo)
        store.transact((draft) => {
          draft.conversationMessages = draft.conversationMessages.filter(
        ...
        await studioCommand("messages", {
          ...
          entityId: target.id,
          expectedVersion: 1,
          reason: "Undid a just-sent inbox message",
        });
        await invalidateStudioDomains(queryClient, ["messaging"]);
      }
  ...
  const sendEmail = async (event: FormEvent) => {
    ...
    try {
      if (isDemo) {
        ...
        store.transact((draft) =>
          draft.outbox.push({
            ...
            status: "queued",
            attempts: 0,
            sendAt: new Date(Date.now() + 8_000).toISOString(),
        ...
        const response = await studioCommand("outbox", {
          command: "manual_send",
          expectedVersion: 0,
          payload: { ...email, studentId: selectedStudent?.id },
          reason: `Manually sent ${emailTemplate.replaceAll("_", " ")} email`,
        });
        setUndoEmail({
          id: response.resource.id,
          until: new Date(response.undoUntil).getTime(),
        });
        await invalidateStudioDomains(queryClient, ["messaging"]);
      }
      setEmailOpen(false);
      ...
      setNotice(
        reason instanceof Error ? reason.message : "Email could not be queued.",
      );
    } finally {
      setBusy(false);
    }
    ...
    try {
      if (isDemo) openEmail(source);
        ...
        const response = await studioCommand("outbox", {
          ...
          reason: "Coach manually resent an email",
        });
        setUndoEmail({
          id: response.resource.id,
          until: new Date(response.undoUntil).getTime(),
      ...
      setNotice(
        reason instanceof Error
          ? reason.message
          : "Email could not be queued again.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const undoLastEmail = async () => {
    ...
    try {
      if (isDemo)
        store.transact((draft) => {
          const item = draft.outbox.find((row) => row.id === target.id);
        ...
        await studioCommand("outbox", {
          ...
          entityId: target.id,
          expectedVersion: 1,
          reason: "Coach undid a manual email send",
        });
        await invalidateStudioDomains(queryClient, ["messaging"]);
      }
      ...
      setNotice(
        reason instanceof Error
          ? reason.message
          : "This email has already started sending.",
      );
    }
  };
  ...
  return (
    <div
      className={
        role === "coach" ? "page inbox-page" : "student-page inbox-page"
      }
    >
      <PageHeader
        title="Inbox"
        action={
          role === "coach" ? (
            <div className="header-actions">
              ...
              <button className="primary-button" onClick={() => openEmail()}>
                <Mail />
                Send email
              </button>
            </div>
      ...
      {(undoMessage || undoEmail) && (
        <div className="undo-send" role="status">
          <span>{undoEmail ? "Email queued" : "Message sent"}</span>
          <button
            onClick={() =>
              void (undoEmail ? undoLastEmail() : undoLastMessage())
            }
          >
            <RotateCcw />
            Undo (
            {Math.max(
              1,
              Math.ceil(((undoEmail || undoMessage)!.until - clock) / 1000),
            )}
            s)
          </button>
        </div>
      ...
      <div className={`inbox-layout ${selected ? "has-selection" : ""}`}>
        <aside className="inbox-threads" aria-label="Conversations">
          ...
          <div className="inbox-filters">
            {(["recent", "unread", "drafts"] as const).map((filter) => (
              <button
                key={filter}
                className={threadFilter === filter ? "active" : ""}
          ...
          {visibleThreads.map((thread) => (
            ...
            >
              ...
              <div>
                <strong>
                  {thread.title}
                  {thread.unread && <i aria-label="Unread" />}
                </strong>
                <small>
                  {thread.draftBody
                    ? `Draft: ${thread.draftBody}`
                    : thread.subtitle}
                </small>
              </div>
            </button>
          ...
          {!visibleThreads.length && (
            <EmptyState
              ...
              detail="Use New message to find any student, household, or group class."
            />
          )}
        </aside>
          ...
          {selected ? (
            <>
              <header>
                ...
                {role === "coach" && selectedStudent?.email && (
              ...
              <div className="inbox-messages" aria-live="polite">
                {messages.map((message) => (
                  <article
                    ...
                    className={
                      ...
                      (role === "coach" && message.authorRole === "coach")
                        ? "mine"
                        : "theirs"
                    }
                  >
                ...
                {!messages.length && (
                  <EmptyState
                    ...
                    detail={
                      selected.kind === "class"
                        ? "Everyone enrolled in this class can read and reply here."
                        : "Messages stay private between the studio and this household."
                    }
                  />
                )}
              </div>
            ...
            <EmptyState
              ...
              detail="Select a student, household, or group class to begin."
            />
          )}
        </section>
      </div>
      {role === "coach" && (
        <section className="inbox-email-history">
          <header>
            <div>
          ...
          <div className="table-list">
            {history.map((item) => (
              <article key={item.id}>
                ...
                {item.eventKey === "campaign.email" ? (
                  <small>Campaign</small>
                ) : (
                  <button
                    disabled={busy}
                    onClick={() => void resendEmail(item)}
                  >
                    Send again
                  </button>
                )}
            ...
            {!history.length && (
              <EmptyState
                ...
                detail="Manually sent and automated email will appear here."
              />
            )}
          </div>
        </section>
          ...
          <div className="recipient-picker">
            ...
            <div>
              {recipients.map((recipient) => (
                <button
                  key={recipient.key}
                  onClick={() => choose(recipient.thread)}
                >
                  <span>
      ...
      {emailOpen && (
        <Dialog
          title="Send email"
          description="The email waits eight seconds before delivery so you can undo an accidental send."
          onClose={() => !busy && setEmailOpen(false)}
        >
          <form className="workflow-form" onSubmit={sendEmail}>
            <label>
              Template
              <select
                value={emailTemplate}
                onChange={(event) =>
                  chooseEmailTemplate(
                    event.target.value as keyof typeof emailTemplates,
                  )
                }
              >
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setEmail({ ...email, recipient: event.target.value })
                }
              />
            </label>
