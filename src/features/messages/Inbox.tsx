...
function Inbox({ data, isDemo, role }: { data: StudioSnapshot; isDemo: boolean; role: Role }) {
  ...
  return <div className={role === "coach" ? "page inbox-page" : "student-page inbox-page"}>
    ...
    {role === "coach" && <section className="inbox-email-history"><header><div><strong>Recent email</strong><small>Individual emails can be sent again. Campaign delivery is managed in Campaigns.</small></div></header><div className="table-list">{history.map((item) => <article key={item.id}><Mail /><div><strong>{item.subject}</strong><small>{item.recipient} · {item.status}</small></div><Status tone={item.status === "sent" ? "good" : item.status === "failed" ? "danger" : "neutral"}>{item.status}</Status>{item.eventKey === "campaign.email" ? <small>Campaign</small> : <button disabled={busy} onClick={() => void resendEmail(item)}>Send again</button>}</article>)}{!history.length && <EmptyState title="No email history" detail="Manually sent and automated email will appear here." />}</div></section>}
