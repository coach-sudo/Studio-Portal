...
export function ActivityCenter({
  ...
  useEffect(() => {
    ...
    void supabase
      ...
      .then(({ data: rows }) =>
        setRead(
          new Set(
            (rows || []).map((row: { event_key: string }) => row.event_key),
          ),
        ),
      );
  }, []);
  ...
  async function clearAll() {
    ...
    setRead(
      (current) => new Set([...current, ...items.map((item) => item.key)]),
    );
    if (!supabase) return;
    const { data: auth } = await supabase.auth.getUser();
    if (!auth.user) return;
  ...
  return (
    <div className="activity-center" ref={root}>
      <button
      ...
      {open && (
        <section className="activity-menu" aria-label="Recent activity">
          <header>
            <div>
              <strong>Recent activity</strong>
              <small>Updates from the last 30 days</small>
            </div>
            <span>
          ...
          <div className="activity-list">
            {visibleFeed.map((item) => {
              ...
              return (
                <button
                  key={item.key}
                  className={`unread ${item.priority}`}
            ...
            {!visibleFeed.length && (
              <p>
                You’re all caught up. New bookings and studio work will appear
                here.
              </p>
            )}
          </div>
