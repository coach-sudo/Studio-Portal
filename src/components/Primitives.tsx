...
export function Toggle({ checked, label, detail, onChange, disabled = false }: { checked: boolean; label: string; detail?: string; onChange: (checked: boolean) => void; disabled?: boolean }) {
  return <button type="button" role="switch" aria-checked={checked} className={`setting-toggle toggle-button ${checked ? "on" : ""}`} disabled={disabled} onClick={() => onChange(!checked)}><span><strong>{label}</strong>{detail && <small>{detail}</small>}</span><i aria-hidden="true"><b /></i></button>;
}
...
export function usePagedList<T>(items: T[], initialSize = 10) {
  ...
  return { visible, page, setPage, pageSize, setPageSize, pageCount, total: items.length };
}

export function ListControls({ page, pageCount, pageSize, total, onPage, onPageSize, label = "items" }: { page: number; pageCount: number; pageSize: number; total: number; onPage: (page: number) => void; onPageSize: (size: number) => void; label?: string }) {
  return <div className="list-controls" aria-label={`${label} display controls`}><label>Show <select value={pageSize} onChange={(event) => { onPageSize(Number(event.target.value)); onPage(1); }}><option value={10}>10</option><option value={25}>25</option><option value={50}>50</option><option value={100}>100</option></select></label><span>{total ? `${(page - 1) * pageSize + 1}–${Math.min(page * pageSize, total)} of ${total}` : `0 ${label}`}</span><div><button type="button" disabled={page <= 1} onClick={() => onPage(page - 1)}>Previous</button><button type="button" disabled={page >= pageCount} onClick={() => onPage(page + 1)}>Next</button></div></div>;
}
export function ExplanationDialog({ title, explanation, evidence, action, onClose, onAction }: { title: string; explanation: string; evidence: string[]; action: string; onClose: () => void; onAction?: () => void }) {
...
export function Dialog({ title, description, children, onClose }: { title: string; description?: string; children: ReactNode; onClose: () => void }) {
  ...
  return <div className="dialog-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section ref={dialog} className="workflow-dialog" role="dialog" aria-modal="true" aria-labelledby="workflow-dialog-title"><header><div><h2 id="workflow-dialog-title">{title}</h2>{description && <p>{description}</p>}</div><button type="button" aria-label="Close" onClick={onClose}>×</button></header>{children}</section></div>;
}
