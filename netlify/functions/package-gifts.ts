...
async function rateLimit(request: Request, action: string) {
  ...
  const { data, error } = await serviceClient().rpc("claim_public_rate_limit", {
    target_key: `package-gift:${action}:${hash(address)}`,
    target_limit: action === "create" ? 8 : 20,
...
export default async (request: Request, context: Context) => {
  ...
  try {
    const action = context.params.action || "catalog",
      db = serviceClient();
    if (request.method === "GET" && action === "catalog") {
      const definitionId =
        new URL(request.url).searchParams.get("definitionId") || "";
      const { data, error } = await db
        .from("package_definitions")
        .select(
          "id,studio_id,name,description,session_count,session_duration_minutes,price_minor,currency,expiration_days,pricing_service_id,delivery_format,giftable,active,visibility,direct_purchase,discount_type,discount_minor,discount_basis_points,recurring_eligible,benefit_text,studios(name,settings)",
        )
        .eq("id", definitionId)
        .eq("active", true)
      ...
      return json({
        ...
        studio: Array.isArray(data.studios) ? data.studios[0] : data.studios,
      });
    }
    ...
    if (action === "create") {
      const definitionId = String(body.definitionId || ""),
        ...
        purchaserEmail = String(body.purchaserEmail || "")
          .trim()
          .toLowerCase(),
        ...
        recipientEmail = String(body.recipientEmail || "")
          ...
          .toLowerCase();
      if (
        purchaserName.length < 2 ||
        recipientName.length < 2 ||
        !purchaserEmail.includes("@") ||
        !recipientEmail.includes("@")
      )
        throw new Error(
          "VALIDATION_FAILED: Complete the purchaser and recipient details.",
        );
      ...
      const { data: gift, error: giftError } = await db
        ...
        .insert({
          ...
          deliver_at: body.deliveryDate
            ? new Date(String(body.deliveryDate)).toISOString()
            : null,
          claim_token_hash: hash(token),
      ...
      const checkout = await new Stripe(key, {
        ...
        {
          idempotencyKey:
            request.headers.get("idempotency-key") || `gift:${gift.id}`,
        },
      );
      await db
        .from("package_gifts")
    ...
    if (action === "claim") {
      ...
      if (token.length < 30 || !email.includes("@") || fullName.length < 2)
        throw new Error("VALIDATION_FAILED: Complete the claim details.");
      const { data: gift, error } = await db
        .from("package_gifts")
        .select("*,package_definitions(*)")
        .eq("claim_token_hash", hash(token))
        .single();
      if (
        error ||
        !gift ||
        gift.recipient_email.toLowerCase() !== email ||
        !["purchased", "delivered"].includes(gift.status) ||
        new Date(gift.expires_at) <= new Date() ||
        (gift.deliver_at && new Date(gift.deliver_at) > new Date())
      )
      ...
      if (!student) {
        const created = await db
          .from("students")
          .insert({
            studio_id: gift.studio_id,
            full_name: fullName,
            email,
            status: "lead",
            portal_enabled: Boolean(body.createPortalProfile),
          })
          .select("id,version")
          .single();
      ...
      if (body.createPortalProfile) {
        ...
        if (account.outboxMessageId)
          context.waitUntil(dispatchOutbox({ ids: [account.outboxMessageId] }));
      }
      return json({
        claimed: true,
        duplicate: Boolean(claimed.data?.duplicate),
        portalRequested: Boolean(body.createPortalProfile),
      });
    }
