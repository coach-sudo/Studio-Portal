...
export default async (request: Request, context: Context) => {
  ...
  try {
    ...
    const content = revision?.content as { displayName?: string; bio?: string; profileLabel?: string; headline?: string; unionStatus?: string; location?: string; playingAge?: string; height?: string; eyeColor?: string; hairColor?: string; website?: string; representation?: string; accentColor?: string; contactEmail?: string; contactPhone?: string; showEmail?: boolean; showPhone?: boolean; primaryHeadshotMaterialId?: string } | undefined;
    ...
    return json({ studio:{name:studio?.name||"Studio",branding,websiteUrl:studio?.settings?.bookingPage?.footerWebsiteUrl},slug: profile.slug, displayName: content?.displayName || profile.display_name, bio: content?.bio || profile.bio, profileLabel:content?.profileLabel, headline:content?.headline, unionStatus:content?.unionStatus, location:content?.location, playingAge:content?.playingAge, height:content?.height, eyeColor:content?.eyeColor, hairColor:content?.hairColor, website:content?.website, representation:content?.representation, accentColor:content?.accentColor, contactEmail:content?.contactEmail, contactPhone:content?.contactPhone, showEmail:Boolean(content?.showEmail), showPhone:Boolean(content?.showPhone), primaryHeadshotMaterialId:content?.primaryHeadshotMaterialId, focusArea: student?.focus_area, publishedAt: revision?.published_at, materials: resolved });
