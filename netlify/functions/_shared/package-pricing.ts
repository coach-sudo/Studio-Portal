...
export function calculatePackagePrice(input: {
  ...
  const basePriceMinor =
    Math.max(0, Math.round(input.unitPriceMinor)) * sessionCount;
  const requested =
    input.discountType === "percent"
      ? Math.round(
          (basePriceMinor *
            Math.min(
              10000,
              Math.max(0, Number(input.discountBasisPoints || 0)),
            )) /
            10000,
        )
      : input.discountType === "fixed"
        ? Math.max(0, Math.round(Number(input.discountMinor || 0)))
        : 0;
...
export async function derivePackageValues(
  ...
  const deliveryFormat =
    payload.deliveryFormat === "in_person" ? "in_person" : "google_meet";
  ...
  if (
    ...
    throw new Error(
      "VALIDATION_FAILED: Choose a service and a valid lesson count.",
    );
  const [
  ...
  if (serviceError || studioError || !service)
    throw (
      serviceError ||
      studioError ||
    ...
    values: {
      ...
      discount_basis_points:
        discountType === "percent"
          ? Number(payload.discountBasisPoints || 0)
          : 0,
      ...
      recurring_eligible:
        ...
        payload.renewalModes.some((mode) => mode !== "one_time"),
