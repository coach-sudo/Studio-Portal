...
| Portal users cannot read coach-only or unrelated-student data                | passed            | GitHub `migrations` passed a fresh reset, lint, and the pgTAP RLS suite, including route RPC and paginated-view isolation. |
| Generated public-schema types match migrations                               | passed            | GitHub `migrations` regenerated the public-schema types and produced a clean diff.                                         |
...
- Supabase reset/lint/pgTAP/type regeneration: passed in the GitHub-hosted `migrations` job. Local execution remains infrastructure-blocked because Docker and Podman are unavailable.
- GitHub `verify`, `migrations`, and deployed `smoke`: passed.
